...
if (parsed.event === "game:goal_scored") {
  ...
  if (ScorerTeam === 0) {
    ...
    GoalScoredContainer.style.backgroundColor = "#" + `${OrangeColorPrimary}`;
    GoalScoredContainer.style.boxShadow =
...
if (parsed.event === "game:statfeed_event") {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      document.getElementById("statfeed-container").removeChild(newEventDiv);
    }, 500);
  }, eventDisplayTimeout);
...
if (parsed.event === "game:update_state") {
  ...
  for (let key in BlueStatBarFIllColors) {
    if (BlueStatBarFIllColors[key]) {
      BlueStatBarFIllColors[key].style.backgroundColor = `#${BlueColorPrimary}`;
    }
  }
...
if (parsed.event === "game:update_state") {
  ...
  if (spectatedPlayer) {
    ...
    const OrangeBackground1 = document.getElementById("teamRight-boost-bg-1");
    const OrangeBackground2 = document.getElementById("teamRight-boost-bg-2");
    const OrangeBackground3 = document.getElementById("teamRight-boost-bg-3");
    ...
    const isOrangeTeam = [OrangePlayer1, OrangePlayer2, OrangePlayer3].includes(
      spectatedPlayerName
    );
...
if (parsed.event === "game:podium_start") {
  ...
  function setPlayerNamesAndStats() {
    ...
    BlueTeamPlayers.forEach((player, index) => {
      const playerElement = document.getElementById(`BluePlayer${index + 1}`);
    ...
    OrangeTeamPlayers.forEach((player, index) => {
      const playerElement = document.getElementById(`OrangePlayer${index + 1}`);
      playerElement.innerHTML = player.name;
      adjustPlayerFontSize(playerElement, player.name);
